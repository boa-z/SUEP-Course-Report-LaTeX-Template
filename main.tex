...
\begin{equation}
    \theta_{k+1}=\phi(\theta_k)=2\theta_k\cos^2\theta_k+\frac{\pi}{2}-\cos\theta_k\sin\theta_k-\frac{\pi}{4} \quad (k=0,1,2,...)
\end{equation}
...
方程(1.2)的根为$\theta=0.9529$，绳长为$R=11.5872$。所以农夫在长满草的半径10米的圆形牛栏，将一头牛栓在栏桩上，但只让牛吃到一半草，栓牛鼻的绳长应为11.5872米。
...
事实上，分段线性插值函数是在两个节点构成的区间上实行$n-1$的拉格朗日插值得到的线性函数。用拉格朗日插值和分段线性插值计算所需的数据，并利用Python编写程序，分别得到坐标$x$每改变0.1时的对应$y$坐标，画出曲线，进行分析。
\subsection{Python代码}
\inputminted[
    frame=lines,
    framesep=2mm,
...
\subsection{结果分析}
采用分段线性插值计算所得结果如图\ref*{img02}所示，其中图像经过一定的后处理。从图中可以看出分段线性插值结果与原始数据吻合较好。
...
Todo: 请于课程最后一次提交报告之前在此处填写不少于500字的读后感
